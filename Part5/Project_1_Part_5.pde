...
class Unistroke {
  ...
  public Unistroke(String n, ArrayList<double[]> unis, int num) {
    name = n;
    sub = num;
    gesture = name.substring(1,name.length()-1);
    if(gesture.charAt(gesture.length()-1)=='1')
...
void draw(){
  ...
  drawButton(190, 10, 50, 20, "Submit");
  
  //Buttons for Sampling data
  //drawButton(0,230,250,20,g);
  //drawButton(0,210,250,20,samp);
...
//   //g = "Result: " + res.name + " (" + ((double)Math.round(res.score*100))  /100 + ")"; 
//   //g = "Gesture Type: " + gestures[count];
